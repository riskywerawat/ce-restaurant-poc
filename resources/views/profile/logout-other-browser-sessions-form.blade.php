<x-jet-action-section>
    ...
    <x-slot name="description">
        {{ __('profile.sessions.subtitle') }}
    </x-slot>

    <x-slot name="content">
        <div class="max-w-xl text-sm">
            {{ __('profile.sessions.text') }}
        </div>
            ...
            <div class="mt-5 space-y-6">
                ...
                @foreach ($this->sessions as $session)
                    <div class="flex items-center">
                        ...
                        <div class="ml-3">
                            <div class="text-sm text-kimberly-inactive">
                                {{ $session->agent->platform() }} - {{ $session->agent->browser() }}
                            </div>

                            <div>
                                <div class="text-xs text-kimberly-secondary">
                                    {{ $session->ip_address }},

                                    @if ($session->is_current_device)
                                        <span class="text-green-500 font-semibold">{{ __('profile.sessions.this_device') }}</span>
                                    @else
        ...
        <x-jet-dialog-modal wire:model="confirmingLogout">
            ...
            <x-slot name="content">
                {{ __('profile.sessions.confirm_text') }}

                <div class="mt-4" x-data="{}" x-on:confirming-logout-other-browser-sessions.window="setTimeout(() => $refs.password.focus(), 250)">
                    <x-jet-input type="password" class="mt-1 block w-3/4" placeholder="{{ __('profile.sessions.password') }}"
                                x-ref="password"
