{{--<div class="flex flex-col p-2 w-full items-center justify-center mb-4">--}}
    {{--<label class="w-48 h-48 sm:w-64 sm:h-64 flex flex-col items-center form-input bg-white uppercase hover:shadow-lg--}}
        ...
        {{--<img id="preview-image" src="{{ $transaction->photo_optimized_raw }}" alt="preview image"--}}
             {{--class="object-cover @if(!$transaction->hasPhoto()) hidden @endif w-full h-full">--}}
        {{--<div class="w-full h-full flex flex-col justify-center items-center @if($transaction->hasPhoto()) hidden @endif" id="upload-photo-icon">--}}
        ...
        {{--</div>--}}
        {{--<div class="absolute right-0 bottom-0 w-8 pr-1 pb-1 flex justify-end @if(!$transaction->hasPhoto()) hidden @endif image-upload-badge" id="upload-photo-badge">--}}
...
<div class="w-full sm:w-1/2 px-2 mb-4">
    @include('dashboard._partials.formfield', [
        'label' => 'ชื่อ', 'name' => 'name', 'type' => 'text', 'value' => $transaction->name ?? old('name'),
        'required' => true
    ])
</div>
<div class="w-full sm:w-1/2 px-2 mb-4">
    @include('dashboard._partials.formfield', [
        'label' => 'อีเมล', 'name' => 'email', 'type' => 'email', 'value' => $transaction->email ?? old('email'),
        'required' => true
    ])
</div>
        ...
        @foreach($roles as $role)
            <label class="w-full sm:w-1/2 md:w-1/3 lg:w-1/4 flex items-center px-2 mb-2 cursor-pointer">
                <input type="checkbox" class="form-checkbox" name="roles[]" value="{{ $role->id }}"
                       @if($transaction->hasRole($role)) checked @endif>
