...
<?php $pageTitle = trans('dashboard/transaction.page_title.create'); ?>
@section('title', $pageTitle)

@section('breadcrumb')
    @include('dashboard._partials.breadcrumb', ['breadcrumbs' => [trans('dashboard/transaction.page_title.index') => route('dashboard.transactions.index')]])
@endsection
...
@section('content')
    <div class="form-container mx-auto my-6">
        ...
        <form class="bg-white shadow px-4 py-5 sm:rounded-lg sm:px-6 my-6" method="post"
              action="{{ route('dashboard.transactions.store') }}">
            @csrf
            <div>
